      ...
      visit(tree, 'element', (node) => {
        if (node.tagName === 'img' && node.properties?.src) {
          ...
          if (src.startsWith('./') || (!src.startsWith('/') && !src.startsWith('http'))) {
            ...
            const imageNameWithoutExt = imageName.replace(/\.(png|jpg|jpeg)$/i, '');
          ...
          const imagePromise = (async () => {
            try {
              ...
              const postsPath = path.join(process.cwd(), 'posts', slug, imagePath.split('/').pop() || '');
              ...
              console.warn(`Could not read image dimensions for ${imagePath}:`, error);
            }
          })();
