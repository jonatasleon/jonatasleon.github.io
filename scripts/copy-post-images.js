...
async function optimizeAndCopyImage(sourcePath, destPath) {
  try {
    ...
    const outputPath = destPath.replace(
      /\.(png|jpg|jpeg)$/i,
      `.${outputFormat}`
    );
    ...
    console.log(
      `Optimized: ${path.basename(sourcePath)} -> ${path.basename(outputPath)} (${savings}% smaller)`
    );
  } catch (error) {
    // If optimization fails, fall back to copying the original
    console.warn(
      `Failed to optimize ${sourcePath}, copying original:`,
      error.message
    );
    fs.copyFileSync(sourcePath, destPath);
  }
...
async function processImages() {
  for (const slug of markdownFiles) {
    ...
    if (
      fs.existsSync(postImageDir) &&
      fs.statSync(postImageDir).isDirectory()
    ) {
...
processImages()
  .then(() => {
    console.log(
      'Finished copying and optimizing post images to public directory'
    );
  })
  .catch((error) => {
